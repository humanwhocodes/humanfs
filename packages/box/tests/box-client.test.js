...
import { BoxClient } from "../src/box-client.js";
...
// Create a new MockServiceWorker worker


const API_BASE = "https://api.box.com/2.0/";

// Define the API endpoints to mock
const API_ENDPOINTS = {
    folders: "folders",
...
describe("BoxClient", () => {
    ...
    describe("findObject()", () => {
        ...
        it("should reject an error if a file cannot be found", async () => {
            const fileOrDirPath = Path.from("/file1.txt");
            ...
            await assert.rejects(() => client.findObject(fileOrDirPath), /ENOENT/);
        });

        it("should reject an error when a folder in the path is not found", async () => {
            const fileOrDirPath = Path.from("/folder1/file1.txt");
    ...
    describe("uploadFile()", () => {
        ...
        // uses same approach as deleteFolder
        it("should pass the correct Authorization and content-type headers", async () => {
        ...
        it("should upload a file with the given name and contents", async () => {
            ...
            server.use(
                http.options(createApiUrl(API_ENDPOINTS.upload), async ({ request }) => {
                    assert.strictEqual(request.headers.get("Authorization"), "Bearer 123");
        ...
        it("should create a new version of an existing file", async () => {
            ...
            server.use(
                http.options(createApiUrl(API_ENDPOINTS.upload), async ({ request }) => {
                    ...
                    const body = await request.json();
                    assert.deepStrictEqual(body, { name: "file1.txt", parent: { id: "0" } });
                ...
                http.post(UPLOAD_URL, async ({ request }) => {
                    ...
                    // check the body formdata
                    const formData = await request.formData();
                    const file = formData.get("file");
                    assert.strictEqual(file.name, "file1.txt");
                    assert.strictEqual(await file.text(), fileContents);
                    assert.strictEqual(formData.get("attributes"), JSON.stringify({ name: "file1.txt", parent: { id: "0" } }));
        ...
        it("should throw an error when the server returns a non-OK response from the preflight call", async () => {
        ...
        it("should throw an error when the server returns a non-OK response from the upload call", () => {
