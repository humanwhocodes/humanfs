...
export class BoxClient {
	...
	async copyFile(fileId, parentId, { name } = {}) {
		...
		const url = new URL(replaceParams(API_ENDPOINTS.copyFile, { file_id: fileId }), this.#apiBase);
	...
	async copyFolder(folderId, parentId, { name } = {}) {
		...
		const url = new URL(replaceParams(API_ENDPOINTS.copyFolder, { folder_id: folderId }), this.#apiBase);
		const response = await this.#fetch(url, {
			method: "POST",
			headers: {
				"Content-Type": "application/json",
	...
	async deleteFile(fileId) {
		const url = new URL(replaceParams(API_ENDPOINTS.updateFile, { file_id: fileId }), this.#apiBase);
		const response = await this.#fetch(url, {
			method: "DELETE"
		});
	...
	async deleteFolder(folderId) {
		const url = new URL(replaceParams(API_ENDPOINTS.updateFolder, { folder_id: folderId }), this.#apiBase);
		const response = await this.#fetch(url, {
			method: "DELETE"
		});
	...
	async download(fileId) {
		...
		const url = new URL(replaceParams(API_ENDPOINTS.file, { file_id: fileId }), this.#apiBase);
		const response = await fetch(url, {
			headers: {
				Authorization: `Bearer ${this.#token}`,
			},
	...
	fetchFolderItems(folderId, { limit = 100, marker = null } = {}) {
		const url = new URL(replaceParams(API_ENDPOINTS.folderItems, { folder_id: folderId }), this.#apiBase);
