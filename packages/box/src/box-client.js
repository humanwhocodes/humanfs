...
import { Path, NotFoundError } from "@humanfs/core";
...
export class BoxClient {
	...
	async copyFile(fileId, parentId, { name } = {}) {
		...
		const url = new URL(replaceParams(API_ENDPOINTS.copyFile, { file_id: fileId }), this.#urlBase);
	...
	async copyFolder(folderId, parentId, { name } = {}) {
		...
		const url = new URL(replaceParams(API_ENDPOINTS.copyFolder, { folder_id: folderId }), this.#urlBase);
		const response = await this.#fetch(url, {
			method: "POST",
			headers: {
				"Content-Type": "application/json",
	...
	async createFolder(name, parentId) {
		...
		const url = new URL("folders", this.#urlBase);
	...
	async deleteFile(fileId) {
		const url = new URL(replaceParams(API_ENDPOINTS.updateFile, { file_id: fileId }), this.#urlBase);
		const response = await this.#fetch(url, {
			method: "DELETE"
		});
	...
	async deleteFolder(folderId) {
		const url = new URL(replaceParams(API_ENDPOINTS.updateFolder, { folder_id: folderId }), this.#urlBase);
		const response = await this.#fetch(url, {
			method: "DELETE"
		});
	...
	async download(fileId) {
		...
		const url = new URL(replaceParams(API_ENDPOINTS.file, { file_id: fileId }), this.#urlBase);
		const response = await fetch(url, {
			headers: {
				Authorization: `Bearer ${this.#token}`,
			},
	...
	fetchFolderItems(folderId, { limit = 100, marker = null } = {}) {
		const url = new URL(replaceParams(API_ENDPOINTS.folderItems, { folder_id: folderId }), this.#urlBase);
	 ...
	 * object entry.
	 * @throws {Error} If the path isn't found.
	 */
	async findObject(fileOrDirPath) {
		...
		for (const entryName of fileOrDirPath) {
			...
			if (!entry) {
				throw new NotFoundError(fileOrDirPath.toString());
			}
	...
	async uploadFile(name, parentId, contents) {
		...
		const preflightUrl = new URL(API_ENDPOINTS.upload, this.#urlBase);
		...
		let preflightResponse = await this.#fetch(preflightUrl, preflightOptions);
		...
		if (!preflightResponse.ok) {
			...
			if (preflight.code === "item_name_in_use") {
				...
				if (!preflightResponse.ok) {
					throw new Error(`${preflightResponse.status} ${preflightResponse.statusText}`);
				}
