...
export class DenoFsxImpl {
     ...
     */ 
    text(filePath) {
        return this.#deno.readTextFile(filePath)
            .catch(error => {
    ...
    arrayBuffer(filePath) {
        return this.#deno.readFile(filePath)
    ...
    async write(filePath, contents) {
        ...
        const op = typeof contents === "string"
            ? () => this.#deno.writeTextFile(filePath, contents, { create: true })
            : () => this.#deno.writeFile(filePath, new Uint8Array(contents), { create: true });

        return op().catch(error => {
                if (error.code === 'ENOENT') {
                    return this.#deno.mkdir(path.dirname(filePath), { recursive: true })
                        .then(op);
                }
     ...
     */
    isFile(filePath) {
        return this.#deno.stat(filePath)
